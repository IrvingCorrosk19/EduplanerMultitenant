...
public class InstitutionalCredentialService : IInstitutionalCredentialService
{
    ...
    public async Task<StaffMemberPublicProfileVm?> ResolvePublicProfileByQrTokenAsync(string rawQrToken)
    {
        ...
        var tokenRow = await _context.Set<StaffQrToken>()
            ...
            .Select(t => new { t.UserId, t.IsRevoked, t.ExpiresAt, t.SchoolId })
            .FirstOrDefaultAsync();
        ...
        var user = await StaffInstitutionalRoleFilter.WhereIsInstitutionalStaff(_context.Users.AsNoTracking().IgnoreQueryFilters())
        ...
        if (user.SchoolId != tokenRow.SchoolId)
            return null;
        ...
        var profile = await _context.Set<StaffInstitutionalProfile>()
            .AsNoTracking()
            .IgnoreQueryFilters()
            .Where(p => p.UserId == tokenRow.UserId && p.SchoolId == tokenRow.SchoolId)
        ...
        var activeCard = await _context.Set<InstitutionalCredentialCard>()
            .AsNoTracking()
            .IgnoreQueryFilters()
            .Where(c => c.UserId == tokenRow.UserId && c.SchoolId == tokenRow.SchoolId && c.Status == "active")
