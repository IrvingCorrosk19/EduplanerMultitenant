...
public class StaffInstitutionalProfileService : IStaffInstitutionalProfileService
{
    ...
    public async Task<bool> UpdateProfileAsync(StaffInstitutionalProfileViewModel model, Guid actorId)
    {
        ...
        if (profile == null)
        {
            if (!user.SchoolId.HasValue)
                return false;
            profile = new StaffProfileEntity { UserId = model.Id, SchoolId = user.SchoolId.Value };
            _context.StaffInstitutionalProfiles.Add(profile);
        }
        else if (profile.SchoolId == Guid.Empty && user.SchoolId.HasValue)
        {
    ...
    public async Task EnsureStaffProfileRowAsync(Guid userId)
    {
        ...
        var schoolId = await _context.Users.AsNoTracking()
            .Where(u => u.Id == userId)
            .Select(u => u.SchoolId)
            .FirstOrDefaultAsync();
        ...
        {
            _logger.LogWarning("[StaffProfile] No se crea fila sin SchoolId UserId={UserId}", userId);
            return;
        }

        _context.StaffInstitutionalProfiles.Add(new StaffProfileEntity { UserId = userId, SchoolId = schoolId.Value });
        await _context.SaveChangesAsync();
